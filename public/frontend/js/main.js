...
var totalPriceVariant = function () {
    var gross_total = $('.fl-sidebar-cart .total.gross');
    ...
    function updateGrossTotal() {
        ...
        $(".tf-product-info-list, .tf-cart-item").each(function () {
            ...
            var totalPrice = parseFloat(productItem.find(".item_total_p").text().replace("$", "")) || 0;
            totalSum += totalPrice;
        });
    ...
    $(".tf-product-info-list, .tf-cart-item").each(function () {
        ...
        productItem.find(".check_price").on("click", function (e) {
          e.preventDefault();
          var newPriceInput = productItem.find("input[name='new_price']");
          var newPrice = parseFloat(newPriceInput.val()) || 0;
          var itemId = newPriceInput.data("item-id");
          ...
          $.ajax({
              ...
              success: function (response) {
                if (response.error) {
                    alert(response.message); // Show error from backend
        ...
        function updateTotalPrice(price, scope) {
            ...
            scope.find(".item_total_p").text(totalPrice.toFixed(2));
            scope.find(".total-price").text(
                "$" + totalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            );

            updateGrossTotal(); // ✅ Update gross total
        }
    });

    updateGrossTotal(); // ✅ Initialize gross total on load
};
...
$(document).ready(function () {
  $(".remove-cart .remove").on("click", function () {
      ...
      $.ajax({
          url: "/cart/remove", // Update with your actual route
          type: "POST",
          data: {
              cart_item_id: cartId,
