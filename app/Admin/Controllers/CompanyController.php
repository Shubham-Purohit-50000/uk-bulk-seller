...
class CompanyController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->model()->whereHas('roles', function ($query) {
            $query->where('name', 'company'); // Change role as per requirement
        });
    ...
    protected function detail($id)
    {
        ...
        $show->franchises('Franchise', function ($franchises) {
            // $franchises->resource('/admin/tie-up-products');
            $franchises->column('id', __('ID'));
            $franchises->column('name', __('Name'));
            $franchises->column('email', __('Email'));
            $franchises->column('phone', __('Phone no.'));
            $franchises->column('latitude', __('Latitude'));
            $franchises->column('longitude', __('Longitude'));
            $franchises->column('address', __('Address'));
            $franchises->column('', __('Type'))->display(function(){
                return '<span class="btn btn-sm btn-danger">' . $this->roles->first()->name . '</span>';
            });
            $franchises->disableActions();
            $franchises->disableCreateButton();
        });
